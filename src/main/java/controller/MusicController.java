...
import domain.LikedMusic;
import domain.LikedMusicRepository;
import domain.Music;
...
public class MusicController {
	...
	@Autowired
	LikedMusicRepository likedMusicRepo;
	...
	public ModelAndView getMusicListFavourite(HttpServletRequest request, HttpServletResponse response)
			throws Exception {

		ModelAndView model = new ModelAndView("favouriteMusicList");
		List<Music> listMusic = loadLikedMusic(request.getRemoteUser());
	...
	public ModelAndView likeSong(@PathVariable("id") Long id, HttpServletRequest request, HttpServletResponse response)
		...
		likedMusic.setIdMusic(id);
		User userCurrent = userRepo.findByUserName(remoteUser);
		likedMusic.setIdUser(userCurrent.getUserid());

		saveLikeToUserAndMusic(likedMusic);
	...
	private void saveLikeToUserAndMusic(LikedMusic likedMusic) {
		likedMusicRepo.save(likedMusic);
	...
	private List<Music> loadLikedMusic(String userName) {
		List<Music> list = new ArrayList<Music>();
		User user = userRepo.findByUserName(userName);

		List<LikedMusic> likedMusicList = likedMusicRepo.findByIdUser(user.getUserid());
		System.out.println(likedMusicList);

		for (LikedMusic likedMusic : likedMusicList) {
			Music music = musicRepo.findOne(likedMusic.getIdMusic());
			list.add(music);
		}
		System.out.println("Liked songs - " + list.size());
		return list;
	}
}
